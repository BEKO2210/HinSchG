...
export function E2eCaseView({ data }: { data: E2eCaseData }) {
  ...
  async function handleUpload(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const result = await encryptAndUpload(
        ...
        `/api/admin/cases/${data.caseId}/e2e-attachments`,
      );
      if (!result.ok) {
  ...
  async function handleDownload(id: string) {
    setError(null);
    const priv = privateKeyRef.current;
    if (!priv || !data.publicKey) {
      setError('Bitte zuerst entsperren.');
      return;
    }
    try {
      const res = await fetch(`/api/admin/cases/${data.caseId}/e2e-attachments/${id}`);
      if (!res.ok) {
      ...
      const e2e = await import('@/lib/e2e');
      const out = await e2e.decryptAttachment(payload, payload.wrap, data.publicKey, priv);
